...
#[no_mangle]
pub extern "C" fn sender_get_commitments(receiver_addr: *const c_char, vks: *const u8, vks_len: u64, commitment_hr: *mut u8, commitment_hr_len: u64, commitment_vks: *mut u8, commitment_vks_len: u64) -> i32 {
    unsafe {
        if receiver_addr.is_null() {
            LAST_ERROR = Some("receiver_addr is null".to_owned());
            return -1;
...
pub extern "C" fn sender_issue_tag(as_tag: *const u8, as_tag_len: u64, randomness_hr: *const u8, randomness_hr_len: u64, randomness_vks: *const u8, randomness_vks_len: u64, vks: *const u8, vks_len: u64, sender_tag: *mut u8, sender_tag_len: u64) -> i32 {
    unsafe {
        ...
        match tag_result {
            Ok(tag) => {
                let sender = SENDER_INSTANCE.as_mut().unwrap();
                let result = sender.get_tag_from_as_tag(tag, randomness_hr_slice, randomness_vks_slice, vks_slice, &mut rng);
                match result {
                    Ok(sender_tag) => {
        ...
        // if epk_len < 32 {
        //     LAST_ERROR = Some("epk_len is not at least 32".to_owned());
        //     return -1;
        // }
        ...
        // let receiver_addr = CStr::from_ptr(receiver_addr).to_str().unwrap();
        // let epk_slice = std::slice::from_raw_parts(epk, epk_len.try_into().unwrap());
        // let commitment_hr_slice = std::slice::from_raw_parts(commitment_hr, commitment_hr_len.try_into().unwrap());
        // let commitment_vks_slice = std::slice::from_raw_parts(commitment_vks, commitment_vks_len.try_into().unwrap());
        ...
        // let tag = sender.get_tag(&channel, )
